...
if(empty($name) || empty($phone)){
    ...
    // параметры запроса
    $data = array(
        'key'             => 'e5a53c6595c1b54b35206d25bf318480-OFF', //Ваш секретный токен
    ...
    if (RETAIL_CRM_USE) {
        ...
        $data_retail = array(
            ...
            'retail_id' => $product_retail_id,
            'quantity' => 1,
            'price' => '',
            'pice_type_id' => '',
            'offer_id' => $product_retail_offer_id,
            'receiverCity'         => $receiverCity,
            'receiverCityRef'      => $receiverCityRef,
            'receiverWarehouseRef' => $receiverWarehouseRef,
            'method_delivery'      => $method_delivery
        );
        $Submit = new Submit();
        $Submit->Init($data_retail);
    }
    // ==============================
    ...
    // ==============================
    if (MONSTERLEADS_USE) {
        require '../vendor/lib/monsterleads.php';
        ...
        $Monsterleads = new MonsterleadsApi();
        $Monsterleads->Init($dataMonster);
    }
    // ==============================
